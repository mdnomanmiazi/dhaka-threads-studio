...
const FeatureBlock: React.FC<FeatureBlockProps> = ({ 
  ...
  return (
    <div 
      ref={blockRef}
      className="animate-on-scroll feature-card relative overflow-hidden rounded-xl transition-all duration-500"
    >
      ...
      <div className="relative h-full flex flex-col items-center justify-between p-6 z-10">
        ...
        <Link 
          to={linkTo} 
          className="explore-link px-6 py-3 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-white transition-all hover:bg-white/20"
        >
          Explore Collection
        </Link>
      </div>
...
const FabricFeatures: React.FC = () => {
  return (
    <section className="py-24 bg-gradient-to-b from-gray-900 to-navy relative overflow-hidden">
      <div className="absolute inset-0 z-0">
        <div className="absolute top-0 left-0 w-full h-full opacity-10">
          ...
          <div className="absolute bottom-20 left-20 w-96 h-96 rounded-full border border-white/10 animate-pulse" style={{ animationDuration: '15s' }}></div>
          <div className="absolute top-1/3 left-1/3 w-32 h-32 rounded-full border-2 border-white/5 animate-spin" style={{ animationDuration: '30s' }}></div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <FeatureBlock 
            ...
            subtitle="Crisp textures and refined details for the perfect structured garments" 
            backgroundImage="https://images.unsplash.com/photo-1594938291221-94f18cbb5660?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80"
            linkTo="/woven"
            delay={0}
            icon={<Shirt size={32} />}
          />
          <FeatureBlock 
            title="KNIT PRECISION" 
            subtitle="Soft, versatile fabrics engineered for comfortable, adaptive wear" 
          ...
          <FeatureBlock 
            ...
            subtitle="Enduring quality meets contemporary style for timeless appeal" 
            backgroundImage="https://images.unsplash.com/photo-1565084888279-aca607ecce0c?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80"
            linkTo="/denim"
            delay={300}
